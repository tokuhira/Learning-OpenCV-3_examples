...
int main(int argc, char** argv) {
    ...
    // results.
    //
    cv::Mat imgA = cv::imread(argv[1], cv::ImreadModes::IMREAD_GRAYSCALE);
    cv::Mat imgB = cv::imread(argv[2], cv::ImreadModes::IMREAD_GRAYSCALE);
    cv::Size img_sz = imgA.size();
    int win_size = 10;
    cv::Mat imgC = cv::imread(argv[2], cv::ImreadModes::IMREAD_UNCHANGED);
