...
int main( int argc, const char** argv )
{
	...
	double rate=capture.get(cv::VideoCaptureProperties::CAP_PROP_FPS);
	Mat MatFrame;
	Mat MatGray;
	Mat MatCanny;
	int delay=1000/rate;
	...
	while(1)
	{
		...
		putText(MatAll,"raw video",Point(50,30),cv::HersheyFonts::FONT_HERSHEY_DUPLEX,1.0f,color);
		putText(MatAll,"gray video",Point(50+MatFrame.cols,30),cv::HersheyFonts::FONT_HERSHEY_DUPLEX,1.0f,color);
		putText(MatAll,"canny video",Point(50+2*MatFrame.cols,30),cv::HersheyFonts::FONT_HERSHEY_DUPLEX,1.0f,color);
